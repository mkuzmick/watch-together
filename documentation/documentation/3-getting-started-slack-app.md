...
2. In the code of the file, change the value of `title` from `Watch Together` to `Slack Tests Go Here`. Note: **do not change** the `index` after `res.render` to `slack` as the `index` is referring to the view `index.ejs`, and we do not yet have a 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjA2MDMzNDQ2MywtMTM1Njg1MjA0MSwxNz
YwMjM3MTcxXX0=
-->